...
Source: VIDEO_PROMPT_WRITING_GUIDE_base_en.md (MiniMax H3 video
prompt-writing guide, community-provided 2026-09; reproduced unmodified
below this header).

How ltxq uses it: this guide is the author's full specification for MiniMax
H3 prompt formats (T2VA / I2VA / FL2VA / L2VA). The keyframe-pair composer
only ever needs the FL2VA branch, so
`templates/llm_directives/minimax-h3-fl2va.txt` is a distilled variant of
section 2.1 (alignment instruction), section 2.2 (three core fields),
section 3.2 (first→last path) and section 4 (shots, camera motion,
soundscape, music) — with the `{{DUR}}` placeholder standing in for the
computed clip duration. `templates/llm_directives/ltx-default.txt` is the
contrasting plain-prose LTX example. Distill further variants from this
file rather than re-deriving the rules from scratch.
