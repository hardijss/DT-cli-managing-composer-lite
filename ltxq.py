...
CREATE TABLE IF NOT EXISTS jobs(
  ...
  backend TEXT, cur_off INT, log_tail TEXT, note TEXT, chain TEXT);
...
def resolve_chain(con, job):
    ...
    slot. Returns a note (also when skipped/omitted), or None. Never fails the
    job: a missing source or an extract error just runs without the frame."""
    slot = job["chain"]
    if not slot:
        return None
    ...
    if not v:
        return "chain: no finished generation to continue from — ran without start frame"
